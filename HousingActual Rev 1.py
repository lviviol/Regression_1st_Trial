...
monthly_stats = df.groupby('date')['resale_price'].agg(['max', 'min', 'median', 'mean']).reset_index()

# Create Line Plot
plt.figure(figsize = (10, 6))
plt.plot(monthly_stats['date'], monthly_stats['max'], color='grey', linestyle='-', label='Max Price')
plt.plot(monthly_stats['date'], monthly_stats['min'], color='orange', linestyle='-', label='Min Price')
plt.plot(monthly_stats['date'], monthly_stats['mean'], color='red', linestyle='-', label='Mean Price')
plt.plot(monthly_stats['date'], monthly_stats['median'], color='green', linestyle='-', label='Median Price')
